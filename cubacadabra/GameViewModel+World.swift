...
extension GameViewModel {
    ...
    internal func handlePresenceEvent(_ event: WorldPresenceEvent) {
        ...
        if event.type == "player_join" {
            remotePlayers[event.playerID] = RemotePlayerState(username: event.username ?? defaultPlayerLabel(event.playerID), position: .zero, yaw: 0, moving: false, sprinting: false, generation: event.generation, appearance: event.appearance)
            remoteRosterDirty = true
    ...
    internal func handleMovementEvent(_ event: WorldMovementEvent) {
        if event.isSelf {
            if event.corrected { engine?.reconcilePlayer(position: event.position, yaw: event.yaw) }
            return
        }
        guard !blockedPlayerIDs.contains(event.playerID) else { return }
    ...
    internal func connectWorld(_ visualWorldID: String) {
        let networkWorldID = visualWorldID == "settings" ? "lobby" : visualWorldID == "real-game" && pendingSessionWorldID != nil ? pendingSessionWorldID! : visualWorldID
    ...
    internal func syncRemotePlayers() {
        guard remoteRosterDirty, let engine else { return }
        let players: [RemoteUpdatePlayer] = worldID == "settings" ? [] : remotePlayers
            .filter { !blockedPlayerIDs.contains($0.key) }
            .sorted { $0.key < $1.key }
            .map { playerID, player in
                RemoteUpdatePlayer(id: playerID, username: player.username, generation: player.generation, position: [player.position.x, player.position.y, player.position.z], yaw: player.yaw, moving: player.moving, sprinting: player.sprinting, appearance: player.appearance)
            }
        remoteSequence &+= 1
        let message = RemoteUpdateMessage(version: 1, sequence: remoteSequence, worldID: worldID == "settings" ? nil : worldID, players: players)
        guard let data = try? JSONEncoder().encode(message), engine.applyRemoteUpdate(data) != 0 else { return }
